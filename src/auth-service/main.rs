...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Here we are using ip 0.0.0.0 so the service is listening on all the configured network
    // interfaces. This is needed for Docker to work, which we will add later on.
    //
